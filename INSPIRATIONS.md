...
- **What we took:**
  ...
  - **Convergence early-exit** — diff successive meeting rounds and stop when stable instead of always running to the cap. → `workflows/crosstalk.py` (live, opt-in `early_exit=True`; status `"converged"`).
  - **Meta-review propagation without fine-tuning** — mine recurring critic concerns and surface them as a STANDING CHECKLIST a caller seeds into the next run (in-context, no weight updates). → `workflows/crosstalk.meta_review_checklist` + `CrosstalkResult.meta_review` (live, deterministic recurrence detection; an LLM `meta_reviewer` role could later distill paraphrased themes).
  ...
  - **Adaptive allocation** — scale round count by *critic spread* (disagreement among critic outputs across rounds), spending more debate on contested findings. → `workflows/crosstalk_policy.rounds_for_spread` + `CrosstalkResult.critic_spread` (live, default-additive).
