...
| Compute plan classifier | PATTERN | Deterministic resource-hint contract: estimate local versus remote compute eligibility with arithmetic only, leaving scheduler commands and checkpoint policy explicit. | `src/vaultlab/projects/compute_plan.py`, `src/vaultlab/projects/compute_plan.md` | MIT-clean; internal planning-contract pattern and no external code copied. | This file plus sibling module markdown. |
| Data inventory schema | PATTERN | Dataset availability contract: path-opaque records summarize available/staged, needs-collection, and restricted inputs before analysis planning. | `src/vaultlab/projects/data_inventory.py`, `src/vaultlab/projects/data_inventory.md` | MIT-clean; internal schema pattern and no external code copied. | This file plus sibling module markdown. |
| Analysis opportunity planner | CONCEPT | Machine-readable gap records replace prose TODOs for missing negative controls, donor-aware support, and absent datasets. | `src/vaultlab/projects/analysis_planning.py`, `src/vaultlab/projects/analysis_planning.md` | MIT-clean; concept reimplemented from the dogfood planning spec. | This file plus sibling module markdown. |
| Lane log handoffs | PATTERN | Read-receipt and lane-handoff records carry context-read evidence plus verification between async project lanes. | `src/vaultlab/projects/lane_log.py`, `src/vaultlab/projects/lane_log.md` | MIT-clean; internal workflow pattern and no external code copied. | This file plus sibling module markdown. |
