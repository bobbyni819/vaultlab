...
- ✅ Criterion #2 (audit-grade enforced): **DONE 2026-05-15**. Invariant suite at `tests/test_vaultlab_invariants/test_red_lines.py` is 8 passed / 0 xfailed. `vaultlab.provenance.write_receipts` wired into every entry in `ARTIFACT_ENTRYPOINTS` (slides/render, figures/publication/save, citations/reporter, manuscript/*, report/dispatch). Every artifact ships with `.provenance.json` + `.method.md` sidecars. CI: `.github/workflows/invariants.yml` runs on every push.
- ✅ Criterion #3 (plug-in companion): **DONE 2026-05-15**. Per-module standalone integration tests at `tests/test_vaultlab_*/test_standalone.py` (8 modules) each invoke their public-API entrypoint from a fresh `tmp_path` with HOME + XDG_CONFIG_HOME isolated and no shared state. All green.
- ✅ Criterion #4 (<30-min onboarding): **DONE 2026-05-15**. `vaultlab demo` produces a real audit-clean artifact in ~1.1s. Clean-VM Dockerfile + CI workflow (`.github/workflows/onboarding.yml`) verify the bar on every push. README features "First run — under 5 minutes" above the fold.
- 🟡 Criterion #5 (composability proven): partial — 3 Bobby-authored example workflows landed in `examples/journal-club/`, `examples/manuscript-section/`, `examples/citation-cleanup/`. The criterion's gate is ≥3 NON-Bobby authored examples; that's the lagging signal awaiting external contributors.
- 🟡 Criterion #1 (adoption signal): channel LIVE at https://github.com/bobbyni819/vaultlab/discussions — pinned welcome thread inviting testimonies. Awaiting first non-Bobby contribution.
