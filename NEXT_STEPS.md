...
> **Reconciliation (2026-06-09, post-merge):** Arnav's `feat/lab-dashboard` branch is now merged into
> `main`, and it CLOSES several items this audit (dated 2026-05-21) lists as unbuilt: **B2** —
> `enforce_hedge` is built and wired into `run_pipeline`; **B3** — `verify_numeric` is built; **B5** —
> `state_aware_preflight` is built; and partially **A2 / A3** (`rigor_audit` methods kind + KB output
> routing). Treat those as DONE. Still genuinely open: the rest of Section B (the `stats/` package,
> the recipe `corpus/sources.json`, per-recipe tests, FigureContract↔recipe wiring) and the Section A
> DECIDE items, which Bobby and Arnav will reconcile together.
