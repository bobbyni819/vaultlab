...
def test_render_report_minimal():
    ...
    assert "Generated by vaultlab.report" in out
    assert _parses(out)
...
def test_status_chip():
    assert 'class="vl-chip good"' in c.status_chip("ok", "good")
    assert 'class="vl-chip bad"' in c.status_chip("fail", "bad")
    # XSS guard
...
def test_mobile_responsive_css_present():
    html_str = render_report(title="x", sections=[])
    assert "@media (max-width: 768px)" in html_str
